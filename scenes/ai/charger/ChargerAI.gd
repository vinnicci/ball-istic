...
func get_new_points() -> void:
	if is_instance_valid(targeting) == false:
		return
	points = level_node.get_points(self, targeting)
...
func chase_target(delta) -> void:
	if bot_node.is_in_control == false:
		return
	if points.size() == 0 || (targeting.global_position - points.back()).length() < 800:
		get_new_points()
...
func back_off(delta) -> void:
	if bot_node.is_in_control == false:
		return
	$VelocityRay.look_at(targeting.global_position)
	bot_node.velocity = Vector2(0,0)
